#########################################################################################
# R script to prepare benchmark data set FlowCAP_WNV
#
# This is a 6-dimensional flow cytometry data set from the FlowCAP-I competition ("West
# Nile virus" data set). See the FlowCAP paper (reference below) for details; see
...
#
# Source: FlowCAP-I competition, "West Nile virus" data set:
...
# - data set available on FlowRepository: https://flowrepository.org/experiments/35 or
# https://flowrepository.org/id/FR-FCM-ZZY3 (or search for "FlowCAP" in FlowRepository)
...
files_data <- list.files("raw_data/FlowRepository_FR-FCM-ZZY3_files", 
                         pattern = "\\.fcs$", full.names = TRUE)

files_labels <- list.files("population_labels_from_Cytobank/FlowCAP-I/Data/Labels/CFSE", 
...
dim(data)  # 1,214,373 cells, 6 dimensions (plus 4 other columns)
table(data[, "label"], exclude = NULL)  # 4 manual gating populations (NA = outliers/unassigned)
table(data[, "sample"])  # 13 samples
...
write.table(data, file = "data/FlowCAP_WNV.txt", quote = FALSE, sep = "\t", row.names = FALSE)


# save data file in .fcs format

flowCore::write.FCS(flowCore::flowFrame(as.matrix(data)), filename = "data/FlowCAP_WNV.fcs")

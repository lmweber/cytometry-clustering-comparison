#########################################################################################
# R script to convert plots to EPS format for journal submission
#
# Lukas M. Weber, April 2016
#########################################################################################

# for journal submission: convert plots to EPS format, max width 7.5in, all fonts as outlines
# requires: Ghostscript and pdftops (Xpdf)



# Figure 1: Levine_32 multi-panel

fn.pdf <- "../plots/Levine_2015_marrow_32/plots_multi_panel_Levine2015marrow32.pdf"
fn.eps <- "../plots_submission/Fig1_Levine32.eps"
fn.tmp.eps <- "../plots_submission/Fig1_Levine32.tmp.eps"

system(paste("pdftops -eps -paperw 2100", fn.pdf, fn.tmp.eps))
system(paste("gs -o", fn.eps, "-sDEVICE=epswrite -dNoOutputFonts -dEPSCrop", fn.tmp.eps))
unlink(fn.tmp.eps)



# Figure 2: Cluster medians for FlowSOM_meta

fn.pdf <- "../plots/Levine_2015_marrow_32/cluster_medians/cluster_medians_heatmap_FlowSOM_meta_Levine2015marrow32.pdf"
fn.eps <- "../plots_submission/Fig2_cluster_medians.eps"
fn.tmp.eps <- "../plots_submission/Fig2_cluster_medians.tmp.eps"

system(paste("pdftops -eps -paperw 1400", fn.pdf, fn.tmp.eps))
system(paste("gs -o", fn.eps, "-sDEVICE=epswrite -dNoOutputFonts -dEPSCrop", fn.tmp.eps))
unlink(fn.tmp.eps)



# Figure 3: Mosmann multi-panel

fn.pdf <- "../plots/Mosmann_2014_activ/plots_multi_panel_Mosmann2014activ.pdf"
fn.eps <- "../plots_submission/Fig3_Mosmann.eps"
fn.tmp.eps <- "../plots_submission/Fig3_Mosmann.tmp.eps"

system(paste("pdftops -eps -paperw 970", fn.pdf, fn.tmp.eps))
system(paste("gs -o", fn.eps, "-sDEVICE=epswrite -dNoOutputFonts -dEPSCrop", fn.tmp.eps))
unlink(fn.tmp.eps)



# Figure 4: Stability analysis for Mosmann

fn.pdf <- "../plots/Mosmann_2014_activ/stability_analysis/stability_boxplots_Mosmann2014activ.pdf"
fn.eps <- "../plots_submission/Fig4_stability.eps"
fn.tmp.eps <- "../plots_submission/Fig4_stability.tmp.eps"

system(paste("pdftops -eps -paperw 970", fn.pdf, fn.tmp.eps))
system(paste("gs -o", fn.eps, "-sDEVICE=epswrite -dNoOutputFonts -dEPSCrop", fn.tmp.eps))
unlink(fn.tmp.eps)




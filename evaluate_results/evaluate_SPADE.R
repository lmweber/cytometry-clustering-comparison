#########################################################################################
# R script to load and evaluate results for SPADE
#
...
RES_DIR_SPADE <- "../../results/manual/SPADE"
...
files_out <- list(
  Levine_32dim = file.path(RES_DIR_SPADE, "SPADE_labels_Levine_32dim.txt"), 
  Levine_13dim = file.path(RES_DIR_SPADE, "SPADE_labels_Levine_13dim.txt"), 
  Samusik_01   = file.path(RES_DIR_SPADE, "SPADE_labels_Samusik_01.txt"), 
  Samusik_all  = file.path(RES_DIR_SPADE, "SPADE_labels_Samusik_all.txt"), 
  Nilsson_rare = file.path(RES_DIR_SPADE, "SPADE_labels_Nilsson_rare.txt"), 
  Mosmann_rare = file.path(RES_DIR_SPADE, "SPADE_labels_Mosmann_rare.txt"), 
  FlowCAP_ND   = file.path(RES_DIR_SPADE, "SPADE_labels_FlowCAP_ND.txt"), 
  FlowCAP_WNV  = file.path(RES_DIR_SPADE, "SPADE_labels_FlowCAP_WNV.txt")
)

clus <- vector("list", length(files_out))
names(clus) <- names(files_out)

# skip data sets Levine_32dim and FlowCAP_WNV due to errors
for (i in 2:7) {

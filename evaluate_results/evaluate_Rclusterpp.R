#########################################################################################
# R script to load and evaluate results for Rclusterpp
#
...
RES_DIR_RCLUSTERPP <- "../../results_manual/Rclusterpp"

DATA_DIR <- "../../../benchmark_data_sets"

# which data sets required subsampling for this method (see parameters spreadsheet)
is_subsampled <- c(TRUE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE, FALSE)
...
files_truth <- list(
  ...
  Levine_13dim = file.path(RES_DIR_RCLUSTERPP, "true_labels_Rclusterpp_Levine_13dim.txt"), 
...
files_out <- list(
  Levine_32dim = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Levine_32dim.txt"), 
  Levine_13dim = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Levine_13dim.txt"), 
  Samusik_01   = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Samusik_01.txt"), 
  Samusik_all  = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Samusik_all.txt"), 
  Nilsson_rare = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Nilsson_rare.txt"), 
  Mosmann_rare = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_Mosmann_rare.txt"), 
  FlowCAP_ND   = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_FlowCAP_ND.txt"), 
  FlowCAP_WNV  = file.path(RES_DIR_RCLUSTERPP, "Rclusterpp_labels_FlowCAP_WNV.txt")
)

#########################################################################################
# R script to load and evaluate results for densityCut (additional results)
#
...
# which set of results to use: automatic or manual number of clusters (see report)
RES_DIR_DENSITYCUT <- "../../../../../clustering_comparison_updates/densityCut/results/auto/densityCut"
...
files_truth_densityCut <- files_truth
clus_truth_densityCut <- clus_truth




###############################
### load densityCut results ###
###############################

# load cluster labels

files_out <- list(
  Levine_32dim = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Levine_32dim.txt"), 
  Levine_13dim = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Levine_13dim.txt"), 
  Samusik_01   = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Samusik_01.txt"), 
  Samusik_all  = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Samusik_all.txt"), 
  Nilsson_rare = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Nilsson_rare.txt"), 
  Mosmann_rare = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_Mosmann_rare.txt"), 
  FlowCAP_ND   = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_FlowCAP_ND.txt"), 
  FlowCAP_WNV  = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_FlowCAP_WNV.txt")
)
...
files_densityCut <- files_out
clus_densityCut <- clus

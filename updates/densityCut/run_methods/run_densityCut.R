#########################################################################################
# R script to run densityCut (additional results)
#
# Lukas Weber, February 2017
#########################################################################################

# Note: densityCut can be installed from BitBucket using "devtools::install_bitbucket()".
# However, installation appears to require a compiler (due to dependencies), which may 
# not be available on all systems.
#devtools::install_bitbucket("jerry00/densitycut_dev")


library(flowCore)
library(densitycut)  # note spelling: "densitycut" not "densityCut"
...
####################################################
### Run densityCut: automatic number of clusters ###
####################################################

# run densityCut with automatic selection of number of clusters (default parameters)
...
# save cluster labels
files_labels <- paste0("../../../../../clustering_comparison_updates/densityCut/results/auto/densityCut/densityCut_labels_", 
                       names(clus), ".txt")
...
write.table(runtimes, 
            file = "../../../../../clustering_comparison_updates/densityCut/results/auto/runtimes/runtime_densityCut.txt", 
            quote = FALSE, sep = "\t")

# save session information
sink(file = "../../../../../clustering_comparison_updates/densityCut/results/auto/session_info/session_info_densityCut.txt")
print(sessionInfo())
sink()

cat("densityCut automatic : all runs complete\n")
